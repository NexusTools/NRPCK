...
#define nrpck_drive_drv_write(device, driver, sector, pos, buffer, len) \
	((int(*)(NRPCKDevice*, uint, uchar, char*, uchar))driver->methods[2])(device, sector, pos, buffer, len)

#define nrpck_drive_drv_fastwrite(device, driver, sector, pos, buffer, len) \
	((int(*)(NRPCKDevice*, uint, uchar, char*, uchar))driver->methods[3])(device, sector, pos, buffer, len)
