...
schar nrpck_drv_modem_connect(NRPCKDevice* device, char* address, uchar index, void(*tick)(void)) {
	...
	while(device->data.modem.incmd == 1) {tick();}
...
int nrpck_drv_modem_read(NRPCKDevice* device, char* buffer, uchar len, uchar index, void(*tick)(void)) {
	if(!tick)
		tick = nrpck_sleep10ms;
	device->data.modem.index = index;
	device->data.modem.inlen = len;
	device->data.modem.incmd = 3;
	while(device->data.modem.incmd == 3) {tick();}
...
int nrpck_drv_modem_write(NRPCKDevice* device, char* buffer, uchar len, uchar index, void(*tick)(void)) {
	if(!tick)
		tick = nrpck_sleep10ms;
	if(index != 0)
	...
	while(device->data.modem.outcmd == 4) {tick();}
